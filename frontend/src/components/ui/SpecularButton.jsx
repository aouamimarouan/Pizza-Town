...
// Helper to resolve CSS variables to hex strings so OGL can parse them
const resolveColor = (colorStr) => {
  if (colorStr.startsWith('var(')) {
    // Attempt to parse CSS variable, fallback to white if impossible
    if (typeof window !== 'undefined') {
      const varName = colorStr.match(/var\(([^)]+)\)/)[1];
      const val = getComputedStyle(document.documentElement).getPropertyValue(varName).trim();
      if (val) {
        // If it returns a hex, return it. If it returns something else, we might have issues,
        // but for our theme it returns hex codes like #D62828.
        return val;
      }
    }
  }
...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    const update = now => {
      ...
      const realLineColor = resolveColor(p.lineColor);
      const realBaseColor = resolveColor(p.baseColor);
      
      try {
          lineC.set(realLineColor);
          baseC.set(realBaseColor);
      } catch (e) {
          lineC.set('#ffffff');
          baseC.set('#525252');
      }
